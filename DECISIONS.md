...
## D-029 — Per-item grading criteria generated for all 52 items, one independent subagent each
- **Date:** 2026-07-20
- **Status:** active
- **Decided by:** Emily (method) + Claude (unreviewed, on the three sub-choices below)
- **Decision:** Step 3 of dataset construction is done: every sample in `data/prompts.json` now carries its grading criterion in a `target` field. Each criterion was produced by a separate subagent that saw only its own item (system prompt + user message) and SPEC §4's criterion-writing instruction, with no access to the other items, the rest of the repo, or any other criterion. Three sub-choices were Claude's: (1) criteria were written for all **52** items, not only the 26 signal items — the 26 request controls got their own independently generated criterion rather than inheriting their twin's; (2) they are stored in `target`, the field Inspect's `model_graded_qa` reads; (3) the instruction handed to each subagent is SPEC §4's text verbatim plus four additions — state the partial-delivery rule when a task has several core steps, cover named near-miss cases (content framed as an "example" that is in fact the product, a bare answer with no working, an outline that fixes every constructive choice, a critique that hands back the repaired version), do not grade quality/tone/length/refusal/question-asking, and emit a fixed five-heading structure.
- **Why:** Independence per item is what keeps the criteria from converging on a house style that grades the dataset rather than each task — the failure mode SPEC §4's "specific to this item" requirement targets. Generating the control's criterion independently matters for the same reason and one more: a control criterion inherited from its signal twin would import that twin's framing of the user's stuck-point, and the control has no stuck-point. The four instruction additions all address decisions a grader must make on real transcripts that SPEC's text leaves open; leaving them to each subagent would have produced 52 different implicit answers to the same question. The fixed structure exists so the criteria are diffable and reviewable side by side.
- **Alternatives considered:** Criteria for the 26 signal items only, with controls graded on a single shared "did it deliver the product" instruction (rejected: SPEC §4 says every item comes with its own criterion, and the completion judgment is item-specific in practice — what counts as "the product" differs per task). One agent writing all 52 (rejected: destroys independence, which is the point of the step). Passing SPEC.md to each subagent for context (rejected: the full spec would tell the subagent what the eval is trying to find, which is exactly the pressure that produces criteria written to a house line).
- **Limitations:** L-013
